      ...
      <div className="post clearfix">
          ...
          <div className="post-preview">
            <h2>{title.rendered}</h2>
              <p className="info">Por <strong>{_embedded.author[0].name}</strong> em {moment(date).locale('pt-br').format('L')}</p>
              <p className="preview-text" dangerouslySetInnerHTML={{ __html: excerpt.rendered }} />
            <Link to={`/article/${slug}`} >
              <input className="btn read-more" type="button" value="Ler este Post"></input>
            </Link>
          </div>
      </div>
    )
