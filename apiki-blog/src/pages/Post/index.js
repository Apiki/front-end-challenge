...
const Post = (props) => {
  ...
  const { post, isFetching } = useSelector(
...
if (isFetching || (typeof post[0] === 'undefined' )) return <h2>Carregando...</h2>;

  return (
    <div>
<button onClick={() => handleRedirect()}>back Home</button>
<h1 dangerouslySetInnerHTML={{__html: post[0].title.rendered}}></h1>
<img src={`${post[0]._embedded['wp:featuredmedia']['0'].source_url}`} alt={post[0].title.rendered} width="300"></img>
